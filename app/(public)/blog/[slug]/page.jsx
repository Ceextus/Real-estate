...
export async function generateMetadata({ params }) {
  ...
  const { data: post } = await supabase
    .from("blog_posts")
    .select("title, excerpt, content, image, author, created_at")
    .eq("slug", slug)
    .eq("published", true)
    .single();
  ...
  const description = (
    ...
    "Read the latest from Andreams Homes."
  )
    .replace(/\n/g, " ")
    .trim()
    .slice(0, 160);
  ...
  return {
    ...
    openGraph: {
      ...
      authors: [post.author || "Andreams Homes"],
      images: [
        {
          url: ogImage,
          width: 1200,
...
export default async function BlogPostPage({ params }) {
  ...
  const jsonLd = {
    ...
    author: {
      "@type": "Person",
      name: post.author || "Andreams Homes",
    },
    publisher: {
      "@type": "Organization",
      name: "Andreams Homes",
      ...
      <main className="min-h-screen bg-primary pt-24 pb-24 overflow-hidden selection:bg-accent selection:text-white">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 relative z-10">
          <div className="w-full h-[40vh] md:h-[55vh] rounded-[30px] md:rounded-[50px] overflow-hidden relative shadow-2xl group">
            <Image
              src={post.image}
              alt={`${post.title} — Andreams Homes Blog`}
              fill
