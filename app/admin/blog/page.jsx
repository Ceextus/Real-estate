      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Create, edit, or remove blog posts and news articles.
          </p>
        </div>
        <button
...
function BlogPostForm({ post = null, onSuccess, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    author: post?.author || "Andreams Homes",
  ...
  const [existingAuxImages, setExistingAuxImages] = useState(
    post?.images || [],
  );
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      author: form.author || "Andreams Homes",
      ...
      <div className="space-y-4 pt-4 border-t border-gray-100">
        <div>
          ...
          <p className="text-xs text-gray-500 mb-3">
            Upload multiple images for the article gallery.
          </p>
          <div
            onDrop={(e) => {
              e.preventDefault();
              handleAuxImagesSelect(e.dataTransfer?.files);
            }}
            onDragOver={(e) => e.preventDefault()}
